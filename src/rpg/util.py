...
def pk(nome: str, grupo: str):
    return {"Nome": {"S": nome}, "Grupo": {"S": grupo}}
...
def getNPCs(grupo: str) -> list:
    res = dynamo.query(
        ...
        ExpressionAttributeValues={":grupo": {"S": grupo}},
    )
    return [r["Nome"]["S"] for r in res["Items"]]


def getNPCInfo(nome: str, grupo: str):
    res = dynamo.get_item(TableName=tabela, Key=pk(nome, grupo))
    r = {}
    for k in res["Item"]:
        r[k] = res["Item"][k]["S"]
    return r
...
def deleteItemNPC(nome: str, grupo: str, atributo: str) -> bool:
    params = pk(nome, grupo)
    dynamo.update_item(
        TableName=tabela, Key=params, UpdateExpression=f"REMOVE {atributo}"
    )
    return True
